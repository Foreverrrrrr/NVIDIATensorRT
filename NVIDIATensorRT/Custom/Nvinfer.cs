...
namespace NVIDIATensorRT.Custom
{
    ...
    public class Nvinfer : DisposableTrtObject
    {
        ...
        {
            NativeMethods.enableDetailedTiming(enable);
        }
        ...
        public void LoadInferenceData(string nodeName, IntPtr dataPtr, int elementCount)
        {
            ...
            unsafe
            {
                ...
                {
                    var status = NativeMethods.copyFloatHostToDeviceByNameZeroCopy(
                        ptr,
                        (sbyte*)pName,
        ...
        /// 获取推理结果（返回浮点数组）
        /// </summary>
        /// <param name="output">输出张量结构，包含了节点维度信息</param>
        /// <param name="nodeName">节点名称</param>
        /// <returns>推理结果数组</returns>
        public unsafe float[] GetInferenceResultArray(ref OutputTensor output, string nodeName)
        {
            int batchSize = output.BatchSize;
            int channels = output.Channels;
            int numDetections = output.NumDetections;
            int length = output.MagnitudeTensor;
            float[] result = new float[length];
            sbyte[] nodeNameSbyte = Encoding.UTF8.GetBytes(nodeName + '\0').Select(b => (sbyte)b).ToArray();
            int[] dims = new int[] { batchSize, channels, numDetections };
            int dimLength = dims.Length;
            fixed (sbyte* namePtr = nodeNameSbyte)
            fixed (int* dimsPtr = dims)
            fixed (float* resultPtr = result)
            {
                HandleException.Handler(NativeMethods.copyFloatDeviceToHostByName(ptr, namePtr, resultPtr, (UIntPtr)length));
            }
            return result;
        }
        ...
        public unsafe InferenceTensor GetInferenceResultPtr(ref OutputTensor output, string nodeName)
        {
            int batchSize = output.BatchSize;
            int channels = output.Channels;
            int numDetections = output.NumDetections;
        ...
        {
            sbyte* namePtr = stackalloc sbyte[nodeName.Length + 1];
            fixed (char* nodeNamePtr = nodeName)
            {
                int byteCount = Encoding.UTF8.GetBytes(nodeNamePtr, nodeName.Length,
                                                      (byte*)namePtr, nodeName.Length + 1);
                namePtr[byteCount] = 0;
            }
            IntPtr hostPtr = IntPtr.Zero;
            ExceptionStatus status = NativeMethods.copyFloatDeviceToHostZeroCopy(
                ptr,
                namePtr,
                ref hostPtr,
                (UIntPtr)output.MagnitudeTensor);
